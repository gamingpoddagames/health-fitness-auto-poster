#!/usr/bin/env python3
"""
FitLife Daily - Quick Image Generator
Creates text-based images instantly - no AI needed!
"""
...
IMAGE_DATA = [
    ...
    {
        "title": "💪 Mindset",
        "text": "Your only competition\nis the person in the mirror",
        "bg": "#1a1a3a",
        "accent": "#7986cb"
    },
    {
        "title": "🥗 Eat Clean",
        "text": "Eat the rainbow!\n5 colors daily for optimal nutrition",
        "bg": "#2a3a1a",
        "accent": "#81c784"
    },
    {
    ...
    },
    {
        ...
        "text": "Motivation gets you started\nHabit keeps you going",
        "bg": "#2a1a1a",
        "accent": "#ef5350"
    }
]

def create_quick_image(template_index, output_dir="images"):
    """Create a text-based image instantly"""
    ...
    # Create background
    image = Image.new('RGB', (width, height), color=data['bg'])
...
def create_all_images(count=25):
    """Create multiple images instantly"""
    print("🎨 Creating Images for FitLife Daily")
    print("=" * 55)
    print("📝 Mode: Text-based images (instant, no AI needed)")
    print()
    ...
    for i in range(count):
        print(f"\n📷 Image {i+1}/{count}")
        filename, data = create_quick_image(i)
        images.append({
            "file": filename,
            "title": data['title'],
            "text": data['text']
    ...
    with open("images/image_list.csv", "w", newline='', encoding='utf-8') as f:
        ...
        for img in images:
            writer.writerow([img['file'], img['title'], img['text']])
